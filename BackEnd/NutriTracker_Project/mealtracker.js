...
document.getElementById('meal-registration-form').addEventListener('submit', function(event) {
    ...
    const intakeDate = document.getElementById('meal-time').value;
    const userId = localStorage.getItem('userId'); // Replace with actual user ID management

    fetch('/api/meal-tracker/track-meal', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mealId, userId, weight, intakeDate })
    })
    .then(response => response.json())
    .then(data => {
        console.log('Meal registered:', data);
        displayResult('Meal successfully registered.');
    })
    .catch(error => console.error('Error registering meal:', error));
});

document.getElementById('ingredient-registration-form').addEventListener('submit', function(event) {
    event.preventDefault();
    const foodItemId = document.getElementById('ingredient-id').value;
    const weight = document.getElementById('ingredient-weight').value;
    const intakeDate = document.getElementById('ingredient-time').value;
    const userId = localStorage.getItem('userId'); // Replace with actual user ID management

    fetch('/api/meal-tracker/track-ingredient', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ foodItemId, userId, weight, intakeDate })
    })
    .then(response => response.json())
    .then(data => {
        console.log('Ingredient registered:', data);
        displayResult('Ingredient successfully registered.');
    })
    .catch(error => console.error('Error registering ingredient:', error));
});
...
document.addEventListener('DOMContentLoaded', function() {
    const userId = localStorage.getItem('userId'); // Antag at brugerens ID gemmes korrekt
    fetch(`/api/meal-tracker/meals/${userId}`)
      .then(response => response.json())
      .then(meals => {
        const dropdown = document.getElementById('meal-select-dropdown');
        meals.forEach(meal => {
          const option = document.createElement('option');
          option.value = meal.id; // Antager at 'id' er kolonnenavn i databasen for måltids ID
          option.textContent = meal.name; // Antager at 'name' er kolonnenavn for måltidsnavn
